...
import { getAmazonSearchUrl } from '../lib/affiliates';
...
export default function WatchProviders({ providers, movieTitle, movieYear, tmdbLink }) {
  ...
  if (!hasSections && !movieTitle) return null;
          ...
          {hasSections && (
            <div className="watch-providers-grid">
              {SECTIONS.map(({ key, label }) => {
                ...
                return (
                  <div key={key} className="watch-providers-group">
                    <h3>{label}</h3>
                    <div className="watch-providers-list">
                      {list.map((p) => (
                        <div key={p.provider_id} className="watch-provider-item">
                          <img
