...
import { getAmazonSearchUrl, getProviderDeepLink, isAmazonProvider } from '../lib/affiliates';
import AffiliateDisclosure from './AffiliateDisclosure';

const TMDB_IMG = 'https://image.tmdb.org/t/p/original';

const SECTIONS = [
  { key: 'ads', label: 'Free with Ads', badge: 'FREE', color: '#22c55e' },
  { key: 'free', label: 'Free', badge: 'FREE', color: '#22c55e' },
  { key: 'flatrate', label: 'Subscription', badge: 'STREAM', color: '#3b82f6' },
  { key: 'rent', label: 'Rent', badge: 'RENT', color: '#f59e0b' },
  { key: 'buy', label: 'Buy', badge: 'BUY', color: '#a855f7' },
];

export default function WatchProviders({ providers, movieTitle, movieYear, tmdbLink }) {
  ...
  const hasSections = providers && SECTIONS.some((s) => providers[s.key]?.length > 0);
  const amazonUrl = getAmazonSearchUrl(movieTitle, movieYear);

  return (
    <div className="watch-providers" ref={ref}>
        ...
        <>
          {hasSections ? (
            <div className="watch-sections">
              {SECTIONS.map(({ key, label, badge, color }) => {
                const list = providers[key];
                if (!list?.length) return null;
                return (
                  <div key={key} className="watch-section">
                    <div className="watch-section-header">
                      <h3>{label}</h3>
                      <span className="provider-badge" style={{ background: `${color}20`, color }}>
                        {badge}
                      </span>
                    </div>
                    <div className="provider-grid">
                      {list.map((p) => {
                        const deepLink = getProviderDeepLink(p.provider_id, movieTitle, movieYear);
                        const cardContent = (
                          <>
                            <img
                              ...
                              loading="lazy"
                            />
                            <span className="provider-card-name">{p.provider_name}</span>
                          </>
                        );

                        if (deepLink) {
                          const amazon = isAmazonProvider(p.provider_id);
                          return (
                            <a
                              key={p.provider_id}
                              href={deepLink}
                              target="_blank"
                              rel={`noopener noreferrer${amazon ? ' sponsored' : ''}`}
                              className="provider-card provider-card-link"
                              onClick={() => {
                                if (typeof window !== 'undefined' && window.gtag) {
                                  window.gtag('event', 'affiliate_click', {
                                    event_category: amazon ? 'Amazon' : 'Streaming',
                                    event_label: `${p.provider_name} — ${movieTitle}`,
                                  });
                                }
                              }}
                            >
            ...
            <div className="no-providers">
              ...
              <p>Not currently available for streaming</p>
              <span>Check platforms directly or search on Amazon</span>
            </div>
          )}

          <div className="watch-providers-links">
            {tmdbLink && (
              <a
                href={tmdbLink}
                target="_blank"
                rel="noopener noreferrer"
                className="watch-providers-justwatch"
              >
            ...
            {amazonUrl && (
              <>
                ...
                >
                  <svg width="70" height="21" viewBox="0 0 602.28 181.499" fill="none" xmlns="http://www.w3.org/2000/svg">
